...
/**
 * TODO write tests that actually check the SIF output is correct (not just
 * that no exceptions happend...)
 * @author rodche
 */
public class SimpleInteractionConverterTest
{
 ...
 static final String outFile = SimpleInteractionConverterTest.class
                                 .getResource(
   "/").getPath() + "simpleInteractionConverterTest.out.txt";
 ...
 public void testWriteInteractionsInSIFNX() throws Exception
 {
  ...
  for (String s : testDir.list(filter))
  {
   ...
   Model level2 = handler.convertFromOWL(in);
   converter.writeInteractionsInSIFNX(level2, out, out, null, Arrays.asList(
     "entity/NAME", "entity/XREF", "entity/ORGANISM"), true);
   in.close();
  }
 }
 ...
 public void testWriteInteractionsInSIFNXl3() throws Exception
 {
  ...
  for (String s : testDir.list(filter))
  {
   ...
   converter.writeInteractionsInSIFNX(m, out, out, Arrays.asList("Entity/name",
                                                                 "Entity/xref"),
                                      Arrays.asList("Entity/xref"), true);
   in.close();
  }
 }
}