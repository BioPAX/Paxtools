...
public class ModelUtils {
	 ...
	 */
	public <T extends Process> void generateEntityProcessXrefsAndComments(
			Class<T> processClass, final String provider) 
	{	
		final String COMMENT = "Auto-generated by Paxtools";
		
		// create or find a special relationship CV
		...
		for(T ownerProc : processes) 
		{
			...
			if (rx == null) {
				...
				// set db, id, cv and comment properties!
				rx.setDb(provider);
	 ...
	 * 
	 * @see #generateEntityProcessXrefsAndComments(Class, String)
	 * 
	 * @param <T>
	 * @param processClass
	 */
	...
	{	
		for(Entity ent : elements) {
			ent.addComment(ownerProc.getModelInterface().getSimpleName()
				+ ":" + ownerProc.getRDFId() 
