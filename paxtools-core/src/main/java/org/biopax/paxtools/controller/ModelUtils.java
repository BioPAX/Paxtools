...
public final class ModelUtils
{
	...
	public static void normalizeGenerics(Model model)
	{
		...
		final HashMap<Set<EntityReference>, EntityReference> memberMap = new HashMap<Set<EntityReference>,
				EntityReference>();
		final Set<SimplePhysicalEntity> pes = model.getObjects(SimplePhysicalEntity.class);
		final Set<SimplePhysicalEntity> pesToBeNormalized = new HashSet<SimplePhysicalEntity>();
	...
	private static void createNewERandAddMembers(Model model, SimplePhysicalEntity pe,
			final HashMap<Set<EntityReference>, EntityReference> memberMap)
	{
		...
		if (er == null)
		{
			...
			if (firstEntityReference != null)
			{
				...
				String syntheticId = model.getXmlBase() + md5hex(pe.getUri());
				...
				er.addComment("auto-generated by Paxtools from generic " 
						+ pe.getModelInterface().getSimpleName() + ", uri=" + pe.getUri());
	...
	public static void copySimplePointers(Model model, Named source, Named target)
	{
		...
		for (Xref xref : source.getXref())
		{
			if (xref instanceof UnificationXref)
			{
				...
				String id = model.getXmlBase() + md5hex(xref.getDb()+xref.getUri());
			...
			ShallowCopy copier)
	{
		...
		for (EntityFeature ef : feature)
		{
			if (ef instanceof BindingFeature)
			{
				...
				{
					System.err.println(
							"The Complex" + complex.getName() + "(" + complex.getUri() + ") has  component" +
							component.getDisplayName() + "(" + component.getUri() + ") which has" +
							"a binding feature (" + ef.getUri() + "), but none of the bound " +
		...
		if (!connected)
		{
			...
			BindingFeature bf = model.addNew(BindingFeature.class,
			                                 component.getUri() + "bond" + "in_Complex_" + complex.getUri());
	...
	private static PhysicalEntity createCopy(Model model, Complex complex, PhysicalEntity component, ShallowCopy copier)
	{
		...
		component = copier.copy(model, component, component.getUri() + "in_Complex_" + complex.getUri());
	...
	private static void inferEntityFromPE(EntityFeature ef, Set<PhysicalEntity> pes)
	{
		...
		for (PhysicalEntity physicalEntity : pes)
		{
			if (physicalEntity instanceof SimplePhysicalEntity)
			{
				...
				{
					er.addEntityFeature(ef);
					LOG.debug("Inferred the ER of " + ef.getUri() + " as " + er.getUri());
					return;
				}
			}
		}
