...
public class Fetcher {
	...
	public static final Filter<PropertyEditor> nextStepFilter =
		editor -> !editor.getProperty().equals("nextStep") && !editor.getProperty().equals("NEXT-STEP");
	...
	public static final Filter<PropertyEditor> evidenceFilter =
		editor -> !editor.getProperty().equals("evidence") && !editor.getProperty().equals("EVIDENCE");
	...
	public static final Filter<PropertyEditor> objectPropertiesOnlyFilter =
		editor -> (editor instanceof ObjectPropertyEditor);
	...
	public Fetcher(EditorMap editorMap, Filter<PropertyEditor>... filters) {
		...
		this.filters = ArrayUtils.add(filters, objectPropertiesOnlyFilter);
	...
	public Set<BioPAXElement> fetch(final BioPAXElement bpe, int depth)
	{
		...
		final Set<BioPAXElement> children = new HashSet<>();

		//create a simple traverser to collect direct child elements:
		Traverser traverser = new Traverser(editorMap,
			(domain, range, model, editor) -> children.add((BioPAXElement) range),
			filters
		);
		...
		if(!children.isEmpty() && --depth > 0) {
			for (BioPAXElement element : new HashSet<>(children)) {
	...
	{
		final Set<T> children = new HashSet<>();
